...
class RNNClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        input = torch.concat(
            (ts_embedding, from_embedding, to_embedding, value_embedding), dim=2
        )
...
class RNNRegressor(nn.Module):
    ...
    def forward(self, x):
        # x: (batch_size, seq_len, 5(ts, from, to, value, latency))
        # batch_size = x.shape[0]
        # seq_len = x.shape[1]
        ...
        from_embedding = self.user_embedding_layer(from_user)  # (batch_size, seq_len, embedding_dim)
        to_embedding = self.user_embedding_layer(to_user)
        ts_embedding = self.time_embedding_layer(ts)  # (batch_size, seq_len, embedding_dim)
        value_embedding = self.value_embedding_layer(value)
        ...
        input = torch.concat(
            (ts_embedding, from_embedding, to_embedding, value_embedding, latency_embedding), dim=2
        )
